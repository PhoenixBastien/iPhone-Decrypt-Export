...
def select_device() -> dict[str, str]:
    ...
    # exit if there are no backups available
    if len(backup_paths) == 0:
        sys.exit("There are no backups available!")
    ...
    for backup_path in backup_paths:
        ...
        plist["Last Backup Date"] = dt.astimezone().strftime("%Y-%m-%d %H:%M")

        # add device info to list of encrypted backups
        encrypted_backups.append([plist[header] for header in headers])

    # exit if there are no encrypted backups available
    if len(encrypted_backups) == 0:
        sys.exit("There are no encrypted backups available!")
...
def export_imessage(backup: EncryptedBackup, export_path: str) -> None:
    ...
    backup.extract_files(
        relative_paths_like="Library/SMS/%", output_folder=".", preserve_folders=True
    )
...
def export_whatsapp(backup: EncryptedBackup, export_path: str) -> None:
    ...
    backup.extract_files(
        domain_like=DomainLike.WHATSAPP, output_folder="WhatsApp", preserve_folders=True
    )
...
def export_history(backup: EncryptedBackup, export_path: str) -> None:
    ...
    backup.extract_file(
        relative_path=RelativePath.SAFARI_HISTORY, output_filename="History.db"
    )
